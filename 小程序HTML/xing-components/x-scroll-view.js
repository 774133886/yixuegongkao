...
Component({
  ...
  methods: {
    ...
    _onTouchmove: function(e){
      if (this.data.scrollTop <= 0 && this.data.translate < 20 && e.touches[0].clientY > this.data.touchStart){
        this.setData({
          translate: e.touches[0].clientY - this.data.touchStart-40
        });
      }
      let targetStatus;
      const status = this.data.pullDownStatus;
    ...
    _onTouchEnd: function (e) {
      // var that = this;
      // if (this.data.translate>-40){
      //   var t = setInterval(function(){
      //     if (that.data.translate==-40){
      //       clearInterval(t);
      //     }
      //     that.setData({
      //       translate: that.data.translate-1
      //     });
      //   },10)
      // }
      const status = this.data.pullDownStatus;
